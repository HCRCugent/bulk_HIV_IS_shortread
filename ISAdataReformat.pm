...
sub annotate_blat_file {
	my ($blat_file,$LTR,$name,$path2hg38,$folder) = @_;
	# Remove the "chr" from the end of the hg38 path for the getseq_fast subroutine.
	$path2hg38 =~ s/chr$//;
	...
	while (<$blat_in>) {
		...
		unless (exists $indicator_results{$coordinate}) {
			...
			my ($indicator, $sequence) = getseq_fast($elements[1],$elements[3], $elements[4], $elements[5],$path2hg38,$LTR, $LTR_junction, \@primer_end_bases);
...
# from an hg38 chromosome fasta file based on the coordinates passed to it.
	
sub getseq_fast {
	
	# Initialize the sequence and coordinate variables.
	my $primer_bases = pop;
	my ($sequence_ID, $chr, $strand, $IS_base, $path2hg38, $LTR) = @_;
	...
	# Modify the name of chromosome unknown to match the fasta file. Note JVEM: commented out since name in blat.txtx file already matches fasta name
	...
	my $file = $path2hg38."$chr.fa";
	my $hg38_in; # Initialize the chromosome file handle.
	if(! open($hg38_in,$file)) { # End the program if a chromosome file can't be found.
	...
	# go get the sequence...
	seek($hg38_in,$startbyte,0);
	my $seq;
	my $basesread=read($hg38_in,$seq,$bytestoread);
	$seq=~s/\s//g;
	$seq = uc($seq);
