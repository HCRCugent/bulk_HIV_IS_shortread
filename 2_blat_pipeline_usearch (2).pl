...
# 2_blat_pipeline.pl
# Dr. Xiaolin Wu & Daria W. Wells. Adpated by Dr. Jolien Vermeire
...
# Adaptation : Replaced uclust with usearch for UMI clustering

use strict;
use warnings;

use lib '/data/gent/vo/001/gvo00120/bulkISS/bulk_IS_illumina_v2_inlineindexPE2/'; # Change this path to the location of your modules.
use ISAbatchFilter;
use ISAblatFilter;
use ISAdataReformat;
...
my @hg38_chrs = (1..22,"X","Y","M","Un"); # List of hg38 chromosomes for alignment
my $path2_11ooc = "/data/gent/vo/001/gvo00120/bulkISS/bulk_IS_illumina_v2_inlineindexPE2/11.ooc"; # Location of 11.ooc file required for BLAT						   		
my $path2blat = "/apps/gent/RHEL9/zen2-ib/software/BLAT/3.7-GCC-12.3.0/bin/blat"; # The path to blat application on hpc
my $path2hg38 = "/data/gent/vo/001/gvo00120/bulkISS/bulk_IS_illumina_v2_inlineindexPE2/ref/hg38/chr"; # The path to the human chromosome files
my $path2HIV = "/data/gent/vo/001/gvo00120/bulkISS/bulk_IS_illumina_v2_inlineindexPE2/HIV1_complete_genome.$chr_ext"; # Path to the HIV genome
my $path2uclust = "/data/gent/vo/001/gvo00120/bulkISS/bulk_IS_illumina_v2_inlineindexPE2/usearch11.0.667_i86linux64"; # Path to the USEARCH program.
...
foreach my $data_file (@ARGV) {
	...
	# Record which file is being analyzed.
 	...
 	# /Applications/Blat/blat /Path/to/hg38db/hg38chromFA/chr1.2bit ./PEread_LTR_NonInternal_linker_long_UniquePairWct_F.fa -ooc=11.ooc PEread_LTR_NonInternal_linker_long_UniquePair_F_chr1.psl  -minScore=16 -stepSize=8
	...
	my $blat_out = "$folder/PEread_LTR_NonInternal_linker_long_UniquePair_"; # The base of the BLAT output file name
 	foreach my $s (qw(F R)) {
 		...
 		for (my $c = 0; $c < @hg38_chrs; $c++) {
 			if ($hg38_chrs[$c] =~ m/Un/) {
 				my $unknown_glob = $path2hg38."Un*".$chr_ext;
 				...
 				@args[1,4] = ("$path2hg38$hg38_chrs[$c].$chr_ext","$blat_out$s"."_chr$hg38_chrs[$c].psl");
 				system("@args"); # Run the BLAT command;
 			}
 		}
	...
	my $blatresults_filtered4="$name"."_blat_annotated_with_fuzz.txt";
 	...
 	ISAdataReformat::annotate_blat_file($blatresults_filtered2,$LTR,$name,$path2hg38,$folder);
	...
	# Sort the fasta file by sequence length for uClust.
 	...
 	# Run uClust.
	system("$path2uclust -cluster_fast $folder/Read1_insert.fa -uc $folder/Read1_insert.uc -id 0.90 -centroids $folder/Read1_centroids.fa");
