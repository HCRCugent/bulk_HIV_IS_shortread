...
# 1_demultiplex_inline1_UMIoffset2.pl
# Dr. Xiaolin Wu & Daria W. Wells. Adapted by Dr. Jolien Vermeire
...
# adaptation : UMI offset changed to 8-17bp instead of 0-9 bp
	...
	SCAN: while (my $lineread1=<$INread1>) { # Begin with read 1
		...
		if($lineread1=~/^@(.*)( 1.*)/) { # Lines beginning with @ indicate sequence name 
			...
			if	($read1_first75 =~ m/$R1_junction/ and $read2_first75 =~ m/TCCGCTTAGAGGACT/) {
				...
				# Get the molecular identifier (MID), bases 8-17 of read 2
				# (PE2 inline index occupies bases 0-7 on AVITI-demultiplexed data).
				my $MID = substr $read2_seq, 8, 10;
